...
const QUICK_CHIPS = ["Кто вы?", "Как проходят уроки?", "Стоимость занятий?"]

const RESPONSES: Record<string, string> = {
  "Кто вы?": "Я репетитор английского языка для школьников! Провожу индивидуальные занятия, помогаю с грамматикой, произношением и подготовкой к ОГЭ/ЕГЭ. Хотите узнать подробнее?",
  "Как проходят уроки?":
    "Уроки проходят онлайн или очно — как удобнее. Каждое занятие адаптировано под уровень и цели ученика: от азов до продвинутого уровня.",
  "Стоимость занятий?":
    "Стоимость зависит от формата и частоты занятий. Напишите мне напрямую — подберём удобный вариант! Откройте раздел «Обо мне» для контактов.",
}

const ACTION_RESPONSES: Record<string, { response: string; action: string }> = {
  "открой галерею": { response: "Открываю галерею успехов учеников!", action: "art" },
  "покажи галерею": { response: "Открываю галерею!", action: "art" },
  "открой резюме": { response: "Открываю мой опыт и образование!", action: "resume" },
  "покажи резюме": { response: "Вот моё резюме!", action: "resume" },
  "открой обо мне": { response: "Открываю страницу обо мне!", action: "about" },
  "покажи обо мне": { response: "Рассказываю о себе!", action: "about" },
  "открой статьи": { response: "Открываю полезные статьи об английском!", action: "writings" },
  "покажи статьи": { response: "Вот мои статьи и советы!", action: "writings" },
}
  ...
  const handleInputSubmit = (e: React.FormEvent) => {
    ...
    if (actionMatch) {
      ...
      const defaultResponse =
        "Отличный вопрос! Напишите «открой обо мне» — расскажу подробнее. Или попробуйте «покажи статьи» для полезных советов по английскому!"
